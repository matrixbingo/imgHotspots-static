...
function SummerHtmlImageMapCreator() {
    ...
    var utils = {
        ...
        rightX : function(x) {
            //console.log('rightX : ' + x + ' - '+app.getOffset('x'));
    ...
    var app = (function() {
        ...
        /* Add mousedown event for svg 拖动画图区域 */
        ...
        function onDocumentKeyDown(e) {
            ...
            switch (e.keyCode) {
                ...
                case KEYS.DELETE: /* DELETE key */
                    if (mode === 'editing' && selected_area) {
                        selected_area.removeDelete();
                        selected_area.removeMsg();
        ...
        function deleteArea(selected_area){
            if (mode === 'editing' && selected_area) {
                selected_area.removeDelete();
                selected_area.removeMsg();
                app.removeObject(selected_area);
        ...
        return {
            ...
            clear : function(){
                ...
                code.hide();
                info.unload();
                document.getElementById('img_cut').innerHTML = '热区个数 : ' + objects.length;
                return this;
            },
    ...
    var info = (function() {
        var form = utils.id('edit_details'),
            ...
            action_type_attr = utils.id('action_type'),
            hover_message_attr = utils.id('hover_message'),
            hover_img_attr = utils.id('hover_img'),
            click_message_attr = utils.id('click_message'),
            click_link_attr = utils.id('click_link'),
        ...
        function save(e) {
            ...
            obj.action_type = action_type_attr.value;
            obj.hover_message = hover_message_attr.value;
            obj.hover_img = hover_img_attr.value;
            obj.click_message = click_message_attr.value;
            obj.click_link = click_link_attr.value;
        ...
        action_type_attr.addEventListener('keydown', function(e) { e.stopPropagation(); }, false);
        hover_message_attr.addEventListener('keydown', function(e) { e.stopPropagation(); }, false);
        hover_img_attr.addEventListener('keydown', function(e) { e.stopPropagation(); }, false);
        click_message_attr.addEventListener('keydown', function(e) { e.stopPropagation(); }, false);
        click_link_attr.addEventListener('keydown', function(e) { e.stopPropagation(); }, false);
        ...
        action_type_attr.addEventListener('change', change, false);
        hover_message_attr.addEventListener('change', change, false);
        hover_img_attr.addEventListener('change', change, false);
        click_message_attr.addEventListener('change', change, false);
        click_link_attr.addEventListener('change', change, false);
        ...
        return {
            load : function(object, new_x, new_y) {
                ...
                action_type_attr.value = object.action_type ? object.action_type : '';
                hover_message_attr.value = object.hover_message ? object.hover_message : '';
                hover_img_attr.value = object.hover_img ? object.hover_img : '';
                click_message_attr.value = object.click_message ? object.click_message : '';
                click_link_attr.value = object.click_link ? object.click_link : '';

                FormUtil.initRadioByVal('imgCut_action_type', 'action_type');
    ...
    var from_html_form = (function() {
        var form = utils.id('from_html_wrapper'),
            ...
            regexp_action_type = / action_type="([\S\s]+?)"/,
            regexp_hover_message = / hover_message="([\S\s]+?)"/,
            regexp_hover_img = / hover_img="([\S\s]+?)"/,
            regexp_click_message = / click_message="([\S\s]+?)"/,
            regexp_click_link = / click_link="([\S\s]+?)"/;
        ...
        function test(str) {
            var result_area,
                ...
                result_title,
                result_action_type,
                result_hover_message,
                result_hover_img,
                result_click_message,
                result_click_link,
                type,
                coords,
                area,
            ...
            if (str) {
                ...
                while (result_area) {
                    ...
                    result_action_type = regexp_action_type.exec(area);
                    if (result_action_type) {
                        action_type = result_action_type[1];
                    } else {
                        action_type = '';
                    }

                    result_hover_message = regexp_hover_message.exec(area);
                    if (result_hover_message) {
                        hover_message = result_hover_message[1];
                    } else {
                        hover_message = '';
                    }

                    result_hover_img = regexp_hover_img.exec(area);
                    if (result_hover_img) {
                        hover_img = result_hover_img[1];
                    } else {
                        hover_img = '';
                    }

                    result_click_message = regexp_click_message.exec(area);
                    if (result_click_message) {
                        click_message = result_click_message[1];
                    } else {
                        click_message = '';
                    }

                    result_click_link = regexp_click_link.exec(area);
                    if (result_click_link) {
                        click_link = result_click_link[1];
                    } else {
                        click_link = '';
                    }
    ...
    var buttons = (function() {
        ...
        function onClearButtonClick(e) {
            ...
            utils.foreach(objs, function(item){
                if(item.msg){
                    item.msg.removeAll();
                };
                if(item.top_left_delete){
                    item.top_left_delete.removeAll();
                }
            });
        ...
        app.onClearButtonClick = onClearButtonClick;
        app.onEditButtonClick = onEditButtonClick;
    })();
    ...
    var Rect = function (x, y){
        ...
        var _this = this,
            params = getParams(x + app.getOffset('x') - 13, y + app.getOffset('y') - 13);
    ...
    Rect.prototype.setCoords = function(params){
        ...
        var y = app.getOffset('y') + params.y + 3,
            x = app.getOffset('x') + params.x + 3;
        //移动消息提示
        if(this.msg){
            this.msg.move(x, y);
        }
        if( this.top_left_delete){
            this.top_left_delete.move(x + params.width - 11, y - 11);
        }
     ...
     * 添加消息提示
     */
    Rect.prototype.addMsg = function(obj) {
        this.msg = obj;
    };
    /**
     * 删除消息提示
     */
    Rect.prototype.removeMsg = function() {
        if(this.msg){
            this.msg.removeAll();
        }
    };
    Rect.prototype.removeDelete = function() {
        if(this.top_left_delete){
    ...
    Circle.prototype.setCoords = function(params){
        ...
        var y = app.getOffset('y') + params.cy - 12,
            x = app.getOffset('x') + params.cx - 31;
        //移动消息提示
        if(this.msg){
            this.msg.move(x, y);
        }
        if(this.top_left_delete){
            this.top_left_delete.move(x + 19, y - params.radius + 3);
        }
     ...
     * 添加消息提示
     */
    Circle.prototype.addMsg = function(obj) {
        this.msg = obj;
    };
    /**
     * 删除消息提示
     */
    Circle.prototype.removeMsg = function() {
        if(this.msg){
            this.msg.removeAll();
        }
    };
    ...
    Circle.prototype.removeDelete = function() {
        if(this.top_left_delete){
            this.top_left_delete.removeAll();;
        }
    };
     ...
     * 添加消息提示
     */
    Polygon.prototype.addMsg = function(obj) {
        this.msg = obj;
    };
    /**
     * 删除消息提示
     */
    Polygon.prototype.removeMsg = function() {
        if(this.msg){
            this.msg.removeAll();
        }
    };
    Polygon.prototype.removeDelete = function() {
        if(this.top_left_delete){
            this.top_left_delete.removeAll();;
        }
    };
