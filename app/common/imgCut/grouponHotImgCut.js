...
HotImgCut.replace = function (json) {
    $(json).each(function (i, item) {
        ...
        item.hover_message = DataUtil.encodeURI(item.hover_message);
        item.hover_img = DataUtil.encodeURI(item.hover_img);
        ;
        item.click_message = DataUtil.encodeURI(item.click_message);
        ;
        item.click_link = DataUtil.encodeURI(item.click_link);
        ;
    });
    return json;
};
...
HotImgCut.checkareas = function (areas) {
    ...
    /*    for(var j = 0; j < areas.length; j++){
     ...
     if(!bool){
     window.parent.toast(window.parent.HotImg.warning + '!!', 1500);
     };
     };*/
...
HotImgCut.ajaxSaveNewImgCut = function (obj) {
    ...
    HotImgCut.getAjaxDatas(url, param, function (resp) {
        if (resp == "timeout") {
            ...
            window.parent.toast('保存失败⊙﹏⊙!!!');
            return;
        } else if (resp == 'success') {
            window.parent.toast('保存成功!');
        }
    ...
    $.ajax(
        {
            ...
            error: function (result) {
                ...
                if (result.responseText == 'success') {
                    window.parent.toast('保存成功!!', 1500);
                } else {
                    window.parent.toast('加载异常，网络服务不给力⊙﹏⊙!!!', 1500);
                }
            },
            success: function (result) {
                try {
                    ...
                    window.parent.toast('加载数据异常 ⊙﹏⊙!!', 1500);
                }
            },
            complete: function (e, t) {
                window.parent.progressClose();
                if (t && t == "timeout") {
                    window.parent.toast('加载超时，网络服务不给力⊙﹏⊙!!', 1500);
                    callBack(t);
                }
            }
        }
